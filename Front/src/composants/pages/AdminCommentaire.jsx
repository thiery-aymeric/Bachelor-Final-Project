import { getAll , deleteCommentaire } from "../../services/commentaire.js"
import {useState , useEffect , useContext } from "react"
import { UserContext } from "../../context/userContext"
import { useNavigate } from "react-router-dom"
import { pagination , dtFrMin } from "../../utils/functions.js";
import PaginationCommentaire from "../commun/PaginationCommentaire.jsx"


function AdminCommentaire(){
    const navigate = useNavigate()
    const {isLogged , isAdmin} = useContext(UserContext)
    const [commentaires , setCommentaires] = useState([]);
    const [pageSize] = useState(5);
    const [pageEncours, setPageEncours] = useState(1);


    const changementPage = (event, pageId) => {
        event.preventDefault();
        setPageEncours(pageId)
    };

    useEffect(function(){
        
        if( isLogged()=== false ){
            navigate("/page401")
            return ; 
        }
        if( isAdmin()=== false ){
            navigate("/commentaire")
            return ; 
        }
        async function init(){
            const commentaires = await getAll()
            setCommentaires(commentaires)
        }
        init(); 
    } , [])

    async function supprimer(id){
        const verif = confirm("êtes vous sûr de vouloir supprimer cet activite")
        if(verif){
            const reponse = await deleteCommentaire(id)
            console.log(reponse)
            if(reponse.msg){
                alert(reponse.msg)
                return ;
            }
            setActivites(reponse);
        }
    }

    const posts = pagination(
        commentaires,
        pageEncours,
        pageSize
      );


    return (<div className="admin">
        <h1>Admin</h1>
        <table>
            <thead>
                <tr>
                    <th className="nom">Nom Commentaire</th>
                    <th className="contenu">Contenu message</th>
                    <th className="action">Action</th>
                </tr>
            </thead>
            <tbody>
                {posts.map(function(commentaire, index){
                    return <tr key={index}>
                        <td>{commentaire.nom}</td>

                        <td>{commentaire.message.slice(0,60) }...</td>
                        
            
                       
                        <td>
                            <span className="btn-delete" onClick={ function(){
                               supprimer(commentaire._id)
                            }}>
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" className="bi bi-x" viewBox="0 0 16 16">
                                <path d="M4.646 4.646a.5.5 0 0 1 .708 0L8 7.293l2.646-2.647a.5.5 0 0 1 .708.708L8.707 8l2.647 2.646a.5.5 0 0 1-.708.708L8 8.707l-2.646 2.647a.5.5 0 0 1-.708-.708L7.293 8 4.646 5.354a.5.5 0 0 1 0-.708"/>
                            </svg>
                            </span>
                        </td>
                    </tr>   
                })}
            </tbody>

        </table>
        <PaginationCommentaire
            totalCommentaires={commentaires.length}
            nbCommentaireParPage={pageSize}
            pageEncours={pageEncours}
            onPageChange={changementPage}
        />
        
        
        

    </div>)

}
export default AdminCommentaire;
/** {isAdmin() && <h2>Gestion des Gestionnaires</h2>} */